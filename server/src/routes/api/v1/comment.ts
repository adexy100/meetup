...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!post)
        return next(
          new ErrorHandler(404, "Unable to comment. Post not found.")
        );
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!comment)
        return next(
          new ErrorHandler(404, "Unable to reply. Comment not found.")
        );
      // check if the Post actually exists
      const post = await Post.findById(comment._post_id);
      if (!post)
        return next(new ErrorHandler(404, "Unable to reply. Post not found."));
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res
        .status(200)
        .send(makeResponseJson({ state, likesCount: likesCount.length }));
    } catch (e) {
      console.log(e);
      next(e);
    }
