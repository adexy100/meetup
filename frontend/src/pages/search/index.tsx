...
import { IError, IPost, IProfile } from "../../types/type";
  ...
  const dispatchSearchPosts = async () => {
    try {
      if (searchQuery && searchType) {
        ...
        setError(null);
        const fetchedPosts = await search({
          q: searchQuery,
          type: "posts",
          offset: postOffset,
        });
  ...
  const dispatchSearchUsers = async () => {
    try {
      if (searchQuery) {
        ...
        const fetchedUsers = await search({
          q: searchQuery,
          offset: userOffset,
        });
    ...
    <div className="min-h-screen pt-20">
      <div className="contain">
        <h2 className="text-lg laptop:text-2xl dark:text-white">
          You searched for: &nbsp;
          <span className="text-indigo-700 dark:text-indigo-400 break-all">
            {searchQuery}
          </span>
        </h2>
        ...
        <ul className="space-x-4 mt-4">
          <li
            className={`inline-block border-b-4 border-transparent px-2 py-4 cursor-pointer font-medium hover:border-indigo-700 text-gray-400 ${
              searchType !== "posts" &&
              " border-indigo-700 text-gray-900 dark:text-white dark:border-indigo-400"
            }`}
