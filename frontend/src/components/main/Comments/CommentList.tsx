...
const CommentList: React.FC<IProps> = ({ comments, updateCommentCallback }) => {
  ...
  const deleteSuccessCallback = (comment: IComment) => {
    if (didMount) {
      ...
      setReplies((oldComments) =>
        oldComments.filter((cmt) => cmt.id !== comment.id)
      );
    }
  };
  ...
  return (
    <TransitionGroup component={null}>
      {replies.map((comment) => (
        <CSSTransition timeout={500} classNames="fade" key={comment.id}>
          <CommentItem openDeleteModal={openModal} comment={comment} />
        </CSSTransition>
      ))}
      {/* ---- DELETE MODAL ---- */}
      <Suspense
        fallback={<LoadingOutlined className="text-gray-800 dark:text-white" />}
      >
        {isOpen && (
          <DeleteCommentModal
            isOpen={isOpen}
