...
const ChatBox: React.FC<IProps> = ({ user, target }) => {
  ...
  const fetchMessages = async () => {
    try {
      setLoading(true);
      const messages = await getUserMessages(target.id, {
  ...
  return (
    <div
      className={`bg-white dark:bg-indigo-1100 p-3 relative w-full h-full laptop:w-20rem laptop:shadow-lg laptop:rounded-t-xl laptop:bottom-0 laptop:right-24 laptop:border laptop:border-gray-400 dark:border-gray-800`}
    >
            ...
            {error.status_code === 404 ? (
              <>
                ...
                <span className="text-gray-400 text-sm">
                  Send a message to {target.username}
                </span>
              </>
            ) : (
          ...
          <>
            <TransitionGroup component={null}>
              {target.chats.map((msg, i) => (
                <CSSTransition timeout={500} classNames="fade" key={msg.id}>
                  <div className="flex flex-col">
                    ...
                    >
                      <div className="flex">
                        ...
                        <Avatar
                          ...
                          size="xs"
                          className={`self-end flex-shrink-0 !bg-cover !bg-no-repeat rounded-full ${
                            msg.isOwnMessage ? "ml-1 order-2" : "mr-1 order-1"
                          }`}
                        />
                        {/*  -- MESSAGE-- */}
                        <span
                          className={`py-2 px-3 break-all text-sm rounded-xl ${
                            msg.isOwnMessage
                              ? "bg-indigo-700 text-white order-1"
                              : "bg-gray-300 order-2"
                          }`}
                        >
                          {msg.text}
                    ...
                    >
                      {/* ---DATE ---- */}
                      <span
                        className={`text-gray-400 text-1xs ${
                          msg.isOwnMessage ? "order-2" : "order-1"
                        }`}
                      >
                      ...
                      {msg.isOwnMessage &&
                        msg.seen &&
                        i === target.chats.length - 1 && (
                          <span
                            className={`text-gray-400 mx-2 text-1xs italic flex-grow ${
                              msg.isOwnMessage ? "order-1" : "order-2"
                            }`}
                          >
                            &nbsp;Seen
                          </span>
